...
// Initialize obvious default values here (e.g. 0 for pointers)
    : OpenGLWindow(parent), modelMesh(0),
      ...
      environmentMap(0), texture(0), normal(0), permTexture(0), pixels(0), mouseButton(Qt::NoButton), auxWidget(0),
      isGPGPU(true), hasComputeShaders(true), blinnPhong(true), transparent(false), eta(1.5), lightIntensity(1.0f), shininess(50.0f), lightDistance(5.0f), groundDistance(0.78),
      ...
      m_indexBuffer(QOpenGLBuffer::IndexBuffer), ground_indexBuffer(QOpenGLBuffer::IndexBuffer)
{
    ...
    connect(timer, &QTimer::timeout, [&](){renderNow();  counter+=1;});
    timer->start(100);
...
void glShaderWindow::openNewTexture() {
    ...
    if (ret == QDialog::Accepted) {
        ...
        normalName = textureName.left(textureName.length()-4) + "_normal.png";
        if (!textureName.isNull()) {
			...
			// the shader wants a normal map. We load one.
			normal = new QOpenGLTexture(QImage(normalName));
			if (normal) {
				normal->setWrapMode(QOpenGLTexture::Repeat);
				normal->setMinificationFilter(QOpenGLTexture::LinearMipMapLinear);
				normal->setMagnificationFilter(QOpenGLTexture::Linear);
				normal->bind(4);
            }
        }
        counter = 0;
...
void glShaderWindow::normalsClicked()
{
    normalMaps = true;
    counter = 0;
    renderNow();
...
void glShaderWindow::goldenClicked()
{
    noise = false;
    counter = 0;
    renderNow();
...
void glShaderWindow::noNormalsClicked()
{
    normalMaps = false;
    counter = 0;
    renderNow();
...
void glShaderWindow::haltonClicked()
{
    noise = true;
    counter = 0;
    renderNow();
...
void glShaderWindow::enableProceduralTexture() {
    procedural = true;
    counter = 0;
    renderNow();
}

void glShaderWindow::disableProceduralTexture() {
    procedural = false;
    counter = 0;
    renderNow();
}
...
QWidget *glShaderWindow::makeAuxWindow()
{
    ...
    QGroupBox *groupBox3 = new QGroupBox("Noise:");
    QRadioButton *noise1 = new QRadioButton("&Golden Noise");
    QRadioButton *noise2 = new QRadioButton("&Halton");
    if (noise) noise2->setChecked(true);
    else noise1->setChecked(true);
    connect(noise1, SIGNAL(clicked()), this, SLOT(goldenClicked()));
    connect(noise2, SIGNAL(clicked()), this, SLOT(haltonClicked()));
    QVBoxLayout *vbox3 = new QVBoxLayout;
    vbox3->addWidget(noise1);
    vbox3->addWidget(noise2);
    groupBox3->setLayout(vbox3);
    buttons2->addWidget(groupBox3);
    outer->addLayout(buttons2);

    QGroupBox *groupBox4 = new QGroupBox("Use Normal Maps:");
    QRadioButton *normals1 = new QRadioButton("&No");
    QRadioButton *normals2 = new QRadioButton("&Yes");
    if (normalMaps) normals2->setChecked(true);
    else normals1->setChecked(true);
    connect(normals1, SIGNAL(clicked()), this, SLOT(noNormalsClicked()));
    connect(normals2, SIGNAL(clicked()), this, SLOT(normalsClicked()));
    QVBoxLayout *vbox4 = new QVBoxLayout;
    vbox4->addWidget(normals1);
    vbox4->addWidget(normals2);
    groupBox4->setLayout(vbox4);
    buttons2->addWidget(groupBox4);
    outer->addLayout(buttons2);

    QGroupBox *groupBox5 = new QGroupBox("Use Procedural Marble:");
    QRadioButton *proc1 = new QRadioButton("&No");
    QRadioButton *proc2 = new QRadioButton("&Yes");
    if (procedural) proc2->setChecked(true);
    else proc1->setChecked(true);
    connect(proc1, SIGNAL(clicked()), this, SLOT(disableProceduralTexture()));
    connect(proc2, SIGNAL(clicked()), this, SLOT(enableProceduralTexture()));
    QVBoxLayout *vbox5 = new QVBoxLayout;
    vbox5->addWidget(proc1);
    vbox5->addWidget(proc2);
    groupBox5->setLayout(vbox5);
    buttons2->addWidget(groupBox5);
    outer->addLayout(buttons2);
...
void glShaderWindow::loadTexturesForShaders() {
    ...
    if ((m_program->uniformLocation("normalTexture") != -1)
        || (hasComputeShaders && compute_program->uniformLocation("normalTexture") != -1)) {
		glActiveTexture(GL_TEXTURE4);
        // the shader wants a normalMap. We load one.
        normal = new QOpenGLTexture(QImage(normalName));
        if (normal) {
            normal->setWrapMode(QOpenGLTexture::Repeat);
            normal->setMinificationFilter(QOpenGLTexture::LinearMipMapLinear);
...
void glShaderWindow::render()
{
    ...
    if (hasComputeShaders) {
        ...
        compute_program->setUniformValue("noise", noise);
        ...
        compute_program->setUniformValue("colorbuffer", 3);
        compute_program->setUniformValue("colorTexture", 0);
        compute_program->setUniformValue("normalTexture", 4);
        compute_program->setUniformValue("counter", counter);
        compute_program->setUniformValue("normalMaps", normalMaps);
        compute_program->setUniformValue("procedural", procedural);
        compute_program->setUniformValue("halton", halton);
